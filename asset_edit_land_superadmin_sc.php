...
// Fetch data associated with the form from the database
...
<head>
  ...
  <title>PMO-LNI | Edit Land Asset</title>
...
<body>
  ...
  <div class="pmo-page-header anim-in">
    <div>
      <div class="page-eyebrow"><i class="fas fa-edit me-1"></i> Edit Land Asset</div>
      <h1>Update <span>Asset</span></h1>
    </div>
    <div style="display:flex;flex-direction:column;align-items:flex-end;gap:10px;position:relative;">
      <div class="pmo-breadcrumb">
        ...
        <i class="fas fa-chevron-right"></i>
        <span style="color:rgba(255,255,255,.9);font-weight:600;">Edit Asset</span>
      </div>
    </div>
  </div>
  ...
  <div class="pmo-form-card anim-in anim-d1">
    <form method="post" action="superadmin_edit_asset_process_land_sc.php">
      <input type="hidden" name="asset_id" value="<?php echo $asset_id; ?>">

      <div class="pmo-form-card-header">
        <div class="pmo-form-card-header-icon" style="background:var(--green-lt);border:1.5px solid var(--green-bd);color:var(--green);">
          <i class="fas fa-edit"></i>
        </div>
        <div>
          <h2>Update Asset Information</h2>
          <small>Modify the fields below and click Update Asset to save</small>
        </div>
      </div>
      ...
      <div class="pmo-form-card-body">
        ...
        <div class="pmo-identity-strip">
          ...
          <div class="pmo-chip"><span>Asset No:</span><?php echo htmlspecialchars($row['asset_no'] ?? '—'); ?></div>
          <div class="pmo-chip"><span>Property No:</span><?php echo htmlspecialchars($row['property_no'] ?? '—'); ?></div>
          <div class="pmo-chip"><span>Article:</span><?php echo htmlspecialchars($row['product_name'] ?? '—'); ?></div>
        </div>
        ...
        <div class="pmo-section anim-in anim-d2">
          ...
          <div class="pmo-section-body">
            <div class="pmo-field">
              ...
              <select class="pmo-select" name="category_id" id="category_id">
                <?php
                include("db_conn.php");
                $res = mysqli_query($conn, "SELECT category_id, category_name FROM category");
                while ($r = mysqli_fetch_assoc($res)) {
                  $selected = ($r['category_id'] == $row['category_id']) ? "selected" : "";
                  echo "<option value='" . htmlspecialchars($r['category_id']) . "' $selected>" . htmlspecialchars($r['category_name']) . "</option>";
                }
                mysqli_close($conn);
                ?>
              </select>
            ...
            <div class="pmo-field">
              ...
              <select class="pmo-select" name="land_id">
                ...
                while ($r = mysqli_fetch_assoc($res)) {
                  $selected = (isset($row['land_id']) && $r['land_id'] == $row['land_id']) ? "selected" : "";
        ...
        <div class="pmo-section land anim-in anim-d3">
          <div class="pmo-section-head"><i class="fas fa-map-marked-alt"></i> Asset Article &amp; Description</div>
          <div class="pmo-section-body">
            <div class="pmo-field">
              <label class="pmo-label">Property Name / Article <span class="req">*</span></label>
              <input type="text" class="pmo-input" name="product_name" placeholder="Product Name" value="<?php echo htmlspecialchars($row['product_name']); ?>" required>
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Asset Number</label>
              <input type="text" class="pmo-input" name="asset_no" placeholder="e.g. LA-2024-001" value="<?php echo htmlspecialchars($row['asset_no']); ?>">
            </div>
            <div class="pmo-field span2">
              <label class="pmo-label">Description</label>
              <textarea class="pmo-textarea" name="specific_description" placeholder="Specific land / property description…"><?php echo htmlspecialchars($row['specific_description']); ?></textarea>
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Acquisition Date</label>
              <input type="text" class="pmo-input" name="product_received" placeholder="YYYY-MM-DD" value="<?php echo htmlspecialchars($row['product_received']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Unit Value</label>
              <div class="pmo-input-prefix">
                <span class="pmo-prefix-text">₱</span>
                <input type="text" class="pmo-input currency-input" name="unit_value" placeholder="0.00" value="<?php echo htmlspecialchars($row['unit_value']); ?>">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="pmo-section vehicle anim-in anim-d4" id="vehicle_form_container" style="display:none;">
          <div class="pmo-section-head"><i class="fas fa-car"></i> Additional Vehicle Details</div>
          <div class="pmo-section-body cols-3">
            <div class="pmo-field">
              <label class="pmo-label">Vehicle Group</label>
              <select class="pmo-select" name="a_location_id">
                <?php
                include("db_conn.php");
                $res = mysqli_query($conn, "SELECT a_location_id, location_name FROM asset_location");
                while ($r = mysqli_fetch_assoc($res)) {
                  $selected = ($r['a_location_id'] == $row['a_location_id']) ? "selected" : "";
                  echo "<option value='" . htmlspecialchars($r['a_location_id']) . "' $selected>" . htmlspecialchars($r['location_name']) . "</option>";
                }
                mysqli_close($conn);
                ?>
              </select>
            ...
            <div class="pmo-field">
              ...
              <input type="text" class="pmo-input" name="brand" placeholder="Enter Asset Brand" value="<?php echo htmlspecialchars($row['brand']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Model</label>
              <input type="text" class="pmo-input" name="model" placeholder="Enter Asset Model" value="<?php echo htmlspecialchars($row['model']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Serial Number</label>
              <input type="text" class="pmo-input" name="serial_no" placeholder="Enter Serial Number" value="<?php echo htmlspecialchars($row['serial_no']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Conduction Sticker</label>
              <input type="text" class="pmo-input" name="conduction_sticker" placeholder="Enter Conduction Sticker" value="<?php echo htmlspecialchars($row['conduction_sticker']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Plate Number</label>
              <input type="text" class="pmo-input" name="plate_no" placeholder="Enter Plate Number" value="<?php echo htmlspecialchars($row['plate_no']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Chassis Number</label>
              <input type="text" class="pmo-input" name="chassis_no" placeholder="Enter Chassis Number" value="<?php echo htmlspecialchars($row['chassis_no']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Engine Number</label>
              <input type="text" class="pmo-input" name="engine_no" placeholder="Enter Engine Number" value="<?php echo htmlspecialchars($row['engine_no']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Year Model</label>
              <input type="text" class="pmo-input" name="year_model" placeholder="Enter Year Model" value="<?php echo htmlspecialchars($row['year_model']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Color</label>
              <input type="text" class="pmo-input" name="color" placeholder="Enter Asset Color" value="<?php echo htmlspecialchars($row['color']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Property Number</label>
              <input type="text" class="pmo-input" name="property_no" value="<?php echo htmlspecialchars($row['property_no']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Unit of Measurement</label>
              <input type="text" class="pmo-input" name="unit_of_measurement" placeholder="set / unit" value="<?php echo htmlspecialchars($row['unit_of_measurement']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Condition</label>
              <input type="text" class="pmo-input" name="product_condition" placeholder="Good / Fair / Damaged" value="<?php echo htmlspecialchars($row['product_condition']); ?>">
            </div>
          </div>
        </div>
        ...
        <div class="pmo-section anim-in anim-d4">
          ...
          <div class="pmo-section-body cols-3">
            <div class="pmo-field">
              ...
              <input type="number" class="pmo-input" name="balance_card" placeholder="0" value="<?php echo htmlspecialchars($row['balance_card']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Quantity On Hand</label>
              <input type="number" class="pmo-input" name="onhand_percount" placeholder="0" value="<?php echo htmlspecialchars($row['onhand_percount']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Shortage / Overage (Qty)</label>
              <input type="number" class="pmo-input" name="so_quantity" placeholder="0" value="<?php echo htmlspecialchars($row['so_quantity']); ?>">
            </div>
            <div class="pmo-field span3">
              <label class="pmo-label">Shortage / Overage (Value)</label>
              <div class="pmo-input-prefix">
                <span class="pmo-prefix-text">₱</span>
                <input type="text" class="pmo-input currency-input" name="so_value" placeholder="0.00" value="<?php echo htmlspecialchars($row['so_value']); ?>">
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="pmo-section acct anim-in anim-d5">
          ...
          <div class="pmo-section-body">
            <div class="pmo-field">
              ...
              <input type="text" class="pmo-input" name="accountable_officer" id="accountable_officer" placeholder="Full name" value="<?php echo htmlspecialchars($row['accountable_officer']); ?>">
            </div>
            <div class="pmo-field">
              <label class="pmo-label">Assigned End User</label>
              <input type="text" class="pmo-input" name="enduser" id="enduser" placeholder="Full name" value="<?php echo htmlspecialchars($row['enduser']); ?>">
            </div>
            <div class="pmo-field span2">
              <label class="pmo-label">Accountable Office</label>
              <input type="text" class="pmo-input" name="whereabouts" placeholder="Office / department / location" value="<?php echo htmlspecialchars($row['whereabouts']); ?>">
            </div>
            <div class="pmo-field span2">
              <label class="pmo-label">Remarks</label>
              <textarea class="pmo-textarea" name="remarks" placeholder="Any additional notes…"><?php echo htmlspecialchars($row['remarks']); ?></textarea>
            </div>
          </div>
        </div>
      ...
      <div class="pmo-form-card-footer">
        ...
        <button type="submit" class="pmo-btn pmo-btn-primary" name="update"><i class="fas fa-save"></i> Update Asset</button>
      </div>
...
<script>
  ...
  function toggleVehicleForm() {
    ...
    document.getElementById('vehicle_form_container').style.display = (name === 'MOTOR VEHICLES') ? 'block' : 'none';
  }
  toggleVehicleForm();
